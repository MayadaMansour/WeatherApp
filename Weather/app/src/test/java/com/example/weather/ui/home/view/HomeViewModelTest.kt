...
class HomeViewModelTest {
    ...
    @Test
    suspend fun get_currentWeather_return_myResponse()= runBlockingTest {
        //given
        var lat=30.0
        var lon=25.0
        var data:MyResponce = current1
        //when
        homeViewModel.getWeatherFromApi(current1.lat,current1.lon,"","","","")
        var result = homeViewModel.currentWeather.value
        when (result) {
            is ApiState.Loading -> {
